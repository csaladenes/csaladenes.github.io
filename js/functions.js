...
colors = ["#007ab3", "#128966", "#e73", "#fc1", "#de2d26", "#dd1c77", "#909", "#008198"];
...
inArrayNCindex = function(val, array) {
    var i = array.length;
    val = val.toLowerCase();
    while (i--) { if (array[i].toLowerCase() == val) { return i; } }
    return -1;
}
...
$mainNav = {
    init: function() {
        $("nav").on("click", "a", function() {
            ...
            }).on("mouseover", "a", function() { $(this).css("background", colors[parseInt($(this).attr("rel").replace("group", ""))]) })
            .on("mouseout", "a", function() {
                if ($group.current != parseInt($(this).attr("rel").replace("group", "")))
    ...
    setActive: function() {
        var $nav = $("nav")
        $nav.children("a").removeClass("navActive").css("background", "inherit");
        $nav.children("[rel='group" + $group.current + "']").addClass("navActive").css("background", colors[$group.current]);
